...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('ban')
        .setDescription(commandDescriptions.ban)
    ...
    async run(bot, interaction) {
        try{ 
            ...
            const playerBannedEmbed = new MessageEmbed()
                .setDescription(`Player **${username}** was banned in Minecraft server **${server.name}**.`)
                .setColor(embedColor) 
                .setTimestamp()
                .setFooter({text:interaction.user.tag, iconURL:interaction.user.displayAvatarURL()})
            return interaction.reply({embeds:[playerBannedEmbed], ephemeral:true});
        } catch(e) {
            console.error(`An error ocurred while running the command 'ban' executed by ${interaction.user.tag}(${interaction.user.id}): ${e}`)
